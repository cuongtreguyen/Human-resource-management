...
const LeaveDetail = () => {
  ...
  const getLeaveTypeName = (type) => {
    const map = {
      CASUAL_LEAVE: 'Nghỉ phép riêng',
      SICK_LEAVE: 'Nghỉ ốm',
      ANNUAL_LEAVE: 'Nghỉ năm',
  ...
  useEffect(() => {
    const loadDetail = async () => {
      try {
        ...
        toast.error('Không thể tải chi tiết đơn nghỉ phép');
        navigate('/leaves');
      } finally {
        setLoading(false);
      }
  ...
  const handleApprove = async () => {
    try {
      ...
      if (ok) {
        ...
        navigate('/leaves');
      } else toast.error('Không thể duyệt đơn nghỉ phép');
    } catch (err) {
      console.error(err);
  ...
  const handleReject = async () => {
    ...
    try {
      const ok = await rejectLeaveRequest(id, rejectReason);
      if (ok) {
        toast.success('🚫 Đã từ chối đơn nghỉ phép');
        navigate('/leaves');
      } else toast.error('Không thể từ chối đơn nghỉ phép');
    } catch (err) {
      console.error(err);
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-6">
              <h1 className="text-2xl font-bold">Chi tiết đơn nghỉ phép</h1>
              <p className="text-blue-100 mt-1">Mã đơn #{leave.id}</p>
            </div>

            <div className="p-6 space-y-6">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-gray-700 font-semibold">Mã nhân viên:</p>
                  <p className="text-lg">{leave.employeeId}</p>
                </div>
                <div>
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(leave.onLeaveStatus)}`}>
                    {getStatusName(leave.onLeaveStatus)}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <p className="text-lg font-semibold">{getLeaveTypeName(leave.onLeaveType)}</p>
                </div>

                <div className="bg-gray-50 p-4 rounded-lg">
                  <div className="flex items-center gap-2 text-gray-600 mb-2">
                    <Clock className="w-5 h-5" />
                    <span className="text-sm font-medium">Thời gian nghỉ</span>
                  </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="flex items-center gap-2 text-gray-600 mb-2">
                  <MessageSquare className="w-5 h-5" />
                  <span className="text-sm font-medium">Lý do nghỉ</span>
                </div>
                <p className="text-gray-900">{leave.reason}</p>
              </div>

              <div className="bg-gray-50 p-4 rounded-lg">
                <p className="text-gray-600 mb-1">Số ngày phép còn lại:</p>
                <p className="text-2xl font-bold text-blue-600">{leave.remainingLeaveDays}</p>
              </div>
            </div>

            <div className="flex justify-end gap-3 p-6 border-t bg-gray-50">
              {leave.onLeaveStatus === 'PENDING' && canApprove ? (
                <>
                  <Button
                    variant="secondary"
                    onClick={() => setShowRejectModal(true)}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full">
            <div className="flex justify-between items-center p-4 border-b">
              <h3 className="text-lg font-bold">Từ chối đơn nghỉ phép</h3>
              <button
            ...
            <div className="p-4">
              <textarea
                ...
                className="w-full border rounded-md p-3 focus:ring-2 focus:ring-red-500 resize-none"
                rows={4}
              />
            </div>
