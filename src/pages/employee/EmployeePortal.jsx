...
import { clearRole, getUserInfo, isAuthenticated } from "../../utils/auth";
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // ƯU TIÊN DÙNG employeeCode (EMP004) → nếu không có thì dùng id số
        const identifier = userInfo.employeeCode || userInfo.id;

        if (!identifier) {
          console.error("Không tìm thấy employeeCode hoặc id trong userInfo:", userInfo);
          setLoading(false);
          return;
        }

        console.log("Đang tải thông tin nhân viên với:", identifier); // EMP004 hoặc 16
        const data = await getEmployeeById(identifier);
        setEmployee(data);
      ...
      <div className="relative z-10 p-4 md:p-8 max-w-7xl mx-auto">
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
              <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight">
                Xin chào{employee?.name ? `, ${employee.name}` : ""}
              </h1>
              <p className="text-orange-100 mt-3 text-lg">
                Chúc bạn một ngày làm việc thật hiệu quả và vui vẻ!
              </p>
          ...
          <Card title="Thông tin cá nhân" className="bg-white/95 backdrop-blur shadow-lg">
            <div className="flex flex-col md:flex-row gap-8">
              ...
              <div className="flex flex-col items-center md:items-start gap-4 md:border-r md:pr-8">
                <div className="w-32 h-32 rounded-full bg-gradient-to-br from-orange-500 to-amber-500 flex items-center justify-center text-5xl font-bold text-white shadow-lg">
                  {employee?.name
                    ? employee.name.split(' ').map(n => n[0]).join('').slice(-2).toUpperCase()
                    : "NV"}
                </div>
                <div className="text-center md:text-left">
                  <h3 className="text-2xl font-bold text-gray-800">
