...
const EmployeeProfile = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Dùng employeeCode nếu có, không thì dùng id
        const identifier = userInfo.employeeCode || userInfo.id;

        if (!identifier) {
          console.error('Không tìm thấy identifier:', userInfo);
          setLoading(false);
          return;
        }
  ...
  // Tên đầy đủ: ưu tiên name → nếu không có thì ghép firstName + lastName
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        {/* Grid thông tin - ĐÚNG 100% FIELD TỪ RESPONSE CỦA BẠN */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
