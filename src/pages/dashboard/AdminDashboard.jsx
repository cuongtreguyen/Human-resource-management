...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        setLoading(true);
        const [main, weekly, departments] = await Promise.all([
          api.getAdminStatistics(),
          api.getWeeklyAttendanceStats(),
          api.getEmployeesByDepartment()
        ]);

        setMainStats(main);
        setWeeklyData(weekly.dailyStatistics || []);
  ...
  // Mảng màu cố định cho phòng ban (đẹp và nhất quán)
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 mb-1">Tổng nhân viên</p>
              <p className="text-2xl font-bold text-gray-900">{mainStats?.activeEmployees || 0}</p>
              <span className="text-xs text-green-600 bg-green-50 px-2 py-0.5 rounded-full">
                Đang làm việc
              </span>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 mb-1">Nhân viên mới tháng này</p>
              <p className="text-2xl font-bold text-gray-900">{mainStats?.newEmployeesThisMonth || 0}</p>
              <div className="flex items-center gap-1 mt-2">
                <TrendingUp className="w-4 h-4 text-green-500" />
                <span className="text-xs text-green-600">+{mainStats?.newEmployeesThisMonth || 0} người</span>
              </div>
            </div>
            <div className="w-12 h-12 bg-green-100 rounded-xl flex items-center justify-center">
              <UserPlus className="w-6 h-6 text-green-600" />
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 mb-1">Đơn nghỉ phép</p>
              <p className="text-2xl font-bold text-gray-900">{mainStats?.leaveRequests?.totalLeaveRequests || 0}</p>
              <span className="text-xs text-orange-600 bg-orange-50 px-2 py-0.5 rounded-full">
                {mainStats?.leaveRequests?.pendingLeaveRequests || 0} chờ duyệt
              </span>
            </div>
            <div className="w-12 h-12 bg-orange-100 rounded-xl flex items-center justify-center">
              <Calendar className="w-6 h-6 text-orange-600" />
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 mb-1">Tổng lương tháng</p>
              <p className="text-xl font-bold text-gray-900">{formatCurrency(mainStats?.totalNetSalaryThisMonth)}</p>
            </div>
            <div className="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center">
              <DollarSign className="w-6 h-6 text-purple-600" />
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500">Đúng giờ tháng này</p>
            <p className="text-lg font-bold text-gray-900">{mainStats?.onTimeCheckInsThisMonth || 0}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500">Đi muộn tháng này</p>
            <p className="text-lg font-bold text-gray-900">{mainStats?.lateCheckInsThisMonth || 0}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500">Vắng mặt tháng này</p>
            <p className="text-lg font-bold text-gray-900">{mainStats?.absentEmployeesThisMonth || 0}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500">Tỷ lệ chấm công TB</p>
            <p className="text-lg font-bold text-gray-900">{mainStats?.averageAttendanceRateThisMonth || 0}%</p>
          </div>
        </div>
      </div>

      {/* Weekly Attendance Chart + Department Distribution */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        {/* Weekly Chart */}
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm p-5 border border-gray-100">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Chấm công tuần này</h3>
            ...
            {weeklyData.map((day, i) => (
              <div key={i} className="flex flex-col items-center gap-2">
                <div className="flex flex-col gap-1">
                  <div
                    className="w-8 bg-green-400 rounded-t"
                    style={{ height: `${(day.onTimeCount / 100) * 120}px` }}
                    title={`Đúng giờ: ${day.onTimeCount}`}
                  />
                  <div
                    className="w-8 bg-yellow-400"
                    style={{ height: `${(day.lateCount / 100) * 120}px` }}
                    title={`Đi muộn: ${day.lateCount}`}
                  />
                  <div
                    className="w-8 bg-red-400 rounded-b"
                    style={{ height: `${(day.absentCount / 100) * 120}px` }}
                    title={`Vắng: ${day.absentCount}`}
                  />
                </div>
                <span className="text-xs text-gray-500">{new Date(day.date).toLocaleDateString('vi-VN', { weekday: 'short' })}</span>
              </div>
            ))}
          </div>
          <div className="flex justify-center gap-6 mt-4 pt-4 border-t">
            <div className="flex items-center gap-2"><div className="w-3 h-3 bg-green-400 rounded"></div><span className="text-xs">Đúng giờ</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 bg-yellow-400 rounded"></div><span className="text-xs">Đi muộn</span></div>
            <div className="flex items-center gap-2"><div className="w-3 h-3 bg-red-400 rounded"></div><span className="text-xs">Vắng</span></div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            {deptData.map((dept, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: departmentColors[i % departmentColors.length] }}></div>
                <div className="flex-1">
                  ...
                  <div className="w-full bg-gray-100 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: totalEmployees ? `${(dept.employeeCount / totalEmployees) * 100}%` : '0%',
                        backgroundColor: departmentColors[i % departmentColors.length]
                      }}
                    />
                  </div>
                </div>
            ...
            {deptData.length === 0 && (
              <p className="text-center text-gray-500 text-sm">Chưa có dữ liệu phòng ban</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Thao tác nhanh</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
          <button onClick={() => navigate('/employees/add')} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-blue-50 hover:bg-blue-100 transition">
            <UserPlus className="w-6 h-6 text-blue-600" />
            <span className="text-xs font-medium text-blue-700">Thêm nhân viên</span>
          </button>
          <button onClick={() => navigate('/attendance')} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-green-50 hover:bg-green-100 transition">
            <Clock className="w-6 h-6 text-green-600" />
            <span className="text-xs font-medium text-green-700">Chấm công</span>
          </button>
          <button onClick={() => navigate('/leaves')} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-orange-50 hover:bg-orange-100 transition">
            <Calendar className="w-6 h-6 text-orange-600" />
            <span className="text-xs font-medium text-orange-700">Nghỉ phép</span>
          </button>
          <button onClick={() => navigate('/payroll')} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-purple-50 hover:bg-purple-100 transition">
            <DollarSign className="w-6 h-6 text-purple-600" />
            <span className="text-xs font-medium text-purple-700">Bảng lương</span>
          </button>
          <button onClick={() => navigate('/reports')} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-indigo-50 hover:bg-indigo-100 transition">
            <PieChart className="w-6 h-6 text-indigo-600" />
            <span className="text-xs font-medium text-indigo-700">Báo cáo</span>
          </button>
        </div>
