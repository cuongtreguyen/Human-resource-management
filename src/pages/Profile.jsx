...
// src/pages/Profile.jsx - HOÀN CHỈNH 100%, ĐÃ CÓ "LOẠI HỢP ĐỒNG" + TIẾNG VIỆT ĐẸP
import React, { useEffect, useState } from 'react';
import {
  User, Mail, Phone, MapPin, Briefcase,
  Building2, CreditCard, Heart, FileText, Users
...
const Profile = () => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        const identifier = userInfo?.employeeCode || userInfo?.id;
  ...
  const formatDate = (dateString) => !dateString ? 'Chưa cập nhật' : new Date(dateString).toLocaleDateString('vi-VN');

  // Giới tính
  const getGenderText = (g) => {
    if (!g) return 'Chưa cập nhật';
    const gender = g.toString().toLowerCase();
    return gender === 'male' || gender === 'nam' ? 'Nam' :
           gender === 'female' || gender === 'nữ' ? 'Nữ' : 'Khác';
  };
  ...
  const getEmployeeTypeText = (type) => {
    ...
    return map[type.toString().toUpperCase()] || type;
  };

  // Loại hợp đồng (nếu BE trả về kiểu string như "PERMANENT", "FIXED_TERM",...)
  ...
  if (loading) return <Layout><div className="flex items-center justify-center min-h-screen"><div className="text-center"><div className="w-16 h-16 border-4 border-purple-600 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div><p className="text-gray-600">Đang tải thông tin...</p></div></div></Layout>;
  if (!employee) return <Layout><div className="flex items-center justify-center min-h-screen"><div className="text-center"><User className="w-16 h-16 text-gray-300 mx-auto mb-4" /><p className="text-gray-600">Không tìm thấy thông tin nhân viên</p></div></div></Layout>;

  const roleColors = {
    admin: { gradient: 'from-blue-500 to-blue-600', text: 'text-blue-600' },
    manager: { gradient: 'from-purple-500 to-purple-600', text: 'text-purple-600' },
    accountant: { gradient: 'from-emerald-500 to-emerald-600', text: 'text-emerald-600' }
  };
  const currentTheme = roleColors[userInfo?.role] || roleColors.admin;
  const fullName = employee.name || `${employee.firstName || ''} ${employee.lastName || ''}`.trim() || 'Nhân viên';
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-gray-50 to-gray-100">
        ...
        <div className="max-w-5xl mx-auto px-6 py-8">
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 mb-6">
            <div className="flex items-center gap-6">
              ...
              <div className="flex-1">
                ...
                <p className={`text-xl ${currentTheme.text} font-semibold mb-3`}>{employee.position || 'Chưa cập nhật'}</p>
                <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                  <div className="flex items-center gap-2">
                    <Building2 className={`w-4 h-4 ${currentTheme.text}`} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2 pb-3 border-b">
                <User className={`w-5 h-5 ${currentTheme.text}`} /> Thông tin cá nhân
              </h3>
              <div className="space-y-1">
                <InfoRow label="Ngày sinh" value={formatDate(employee.dateOfBirth)} />
                <InfoRow label="Giới tính" value={getGenderText(employee.gender)} />
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2 pb-3 border-b">
                <Mail className="w-5 h-5 text-blue-600" /> Thông tin liên hệ
              </h3>
              <div className="space-y-1">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2 pb-3 border-b">
                <CreditCard className="w-5 h-5 text-green-600" /> Giấy tờ tùy thân
              </h3>
              <div className="space-y-1">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2 pb-3 border-b">
                <Briefcase className={`w-5 h-5 ${currentTheme.text}`} /> Thông tin công việc
              </h3>
              <div className="space-y-1">
                <InfoRow label="Phòng ban" value={employee.department} />
                <InfoRow label="Chức vụ" value={employee.position} />
                <InfoRow label="Quản lý trực tiếp" value={employee.manager} />
                <InfoRow label="Địa điểm làm việc" value={employee.workLocation} />
                <InfoRow label="Loại nhân viên" value={getEmployeeTypeText(employee.employeeType)} />
                <InfoRow label="Loại hợp đồng" value={getContractTypeText(employee.contractType)} /> {/* ĐÃ CÓ */}
                <InfoRow label="Mã hợp đồng" value={employee.contractCode} /> {/* ĐÃ CÓ */}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2 pb-3 border-b">
                <MapPin className="w-5 h-5 text-red-600" /> Địa chỉ
              </h3>
              <div className="space-y-1">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:col-span-2">
              <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2 pb-3 border-b">
                <Heart className="w-5 h-5 text-red-600" /> Liên hệ khẩn cấp
              </h3>
              {employee.emergencyContact ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-gray-50 rounded-lg p-4"><p className="text-sm text-gray-500 mb-1">Họ và tên</p><p className="text-gray-900 font-medium">{employee.emergencyContact.name}</p></div>
                  <div className="bg-gray-50 rounded-lg p-4"><p className="text-sm text-gray-500 mb-1">Mối quan hệ</p><p className="text-gray-900 font-medium">{employee.emergencyContact.relationship}</p></div>
                  <div className="bg-gray-50 rounded-lg p-4"><p className="text-sm text-gray-500 mb-1">Số điện thoại</p><p className="text-gray-900 font-medium">{employee.emergencyContact.phone}</p></div>
                </div>
              ) : (
                <div className="text-center py-8 text-gray-500">
                  <Users className="w-12 h-12 mx-auto mb-3 text-gray-300" />
          ...
          <div className="mt-6 bg-blue-50 border border-blue-200 rounded-xl p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium text-blue-900">Cần cập nhật thông tin?</p>
                  <p className="text-sm text-blue-700 mt-1">Nếu có thay đổi về thông tin cá nhân, vui lòng gửi yêu cầu để phòng Nhân sự hỗ trợ cập nhật.</p>
                </div>
              </div>
              <a href="/admin/support-tickets" className="flex-shrink-0 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium text-sm">
                Liên hệ Admin
              </a>
            </div>
          </div>
