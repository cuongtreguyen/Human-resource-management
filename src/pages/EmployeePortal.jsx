      ...
      <div className="p-0">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-purple-700 text-white p-6 rounded-2xl mb-8 shadow">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold">Xin chào{employee ? `, ${employee.name}` : ''} 👋</h1>
              <p className="text-purple-100 mt-2">Chúc bạn một ngày làm việc hiệu quả!</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Button variant="secondary" onClick={() => navigate('/employee/leave')}>Xin nghỉ phép</Button>
              <Button variant="outline" onClick={() => navigate('/employee/tasks')}>Xem nhiệm vụ</Button>
              <Button variant="primary" onClick={() => navigate('/employee/attendance')}>Chấm công</Button>
              <Button variant="outline" onClick={() => navigate(-1)}>Quay lại</Button>
              <Button variant="danger" onClick={() => { clearRole(); navigate('/login'); }}>Đăng xuất</Button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card>
            <div className="flex items-center justify-between">
              ...
              <div className="w-10 h-10 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center">🏢</div>
            </div>
          </Card>
          <Card>
            <div className="flex items-center justify-between">
          ...
          <Card>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">{employee ? `${employee.salary?.toLocaleString()}₫` : '-'}</p>
              </div>
        ...
        <Card title="Tác vụ nhanh">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {quickActions.map((qa) => (
              ...
              >
                <div className="text-lg font-semibold text-gray-900">{qa.title}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
          <div className="lg:col-span-2 space-y-6">
            <Card title="Tổng quan nhiệm vụ">
              ...
              <div className="mt-4 flex gap-3">
                <Button variant="primary" onClick={() => navigate('/employee/tasks')}>Xem nhiệm vụ</Button>
                <Button variant="outline" onClick={() => navigate('/employee/chat')}>Trao đổi với quản lý</Button>
              </div>
            </Card>

            <Card title="Thông báo">
              <div className="space-y-3">
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-blue-800">Bảng lương tháng mới đã có sẵn.</div>
                <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-800">Bạn có 1 nhiệm vụ cần hoàn thành trong hôm nay.</div>
                <div className="p-3 bg-purple-50 border border-purple-200 rounded-lg text-purple-800">Cuộc họp phòng vào 15:00 chiều nay.</div>
              </div>
            </Card>
          </div>
          ...
          <div className="space-y-6">
            <Card title="Thông tin cá nhân">
              ...
              <div className="mt-4">
                <Button variant="secondary" onClick={() => navigate('/profile')}>Cập nhật hồ sơ</Button>
              </div>
            </Card>
          </div>
        </div>
